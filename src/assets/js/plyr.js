/**
 * Created by shangxin on 2018-10-29 15:55 export const Plyr =
 */



export const Plyr = function () {
  ...
  var controls = {
    ...
    findElements: function findElements() {
      try {
        this.elements.controls = getElement.call(this, this.config.selectors.controls.wrapper); // Buttons  jack22222
    ...
    create: function create(data) {
      ...
      var container = createElement('div', getAttributesFromSelector(this.config.selectors.controls.wrapper)); // Restart button jack
      //模拟控制条的的显示   用来显示标题 jack
      if (!this.elements.videoTitle) {
        let titleElement = document.createElement('div');
        titleElement.className = 'plyr-video-title plyr__controls';
        titleElement.innerHTML = this.videotitle;
        this.elements.videoTitle = titleElement;
        this.elements.container.appendChild(this.elements.videoTitle);
      }
      //海报时候外面的title
      //视频封面时候的标题
      let videoTitleMark = document.createElement('div');
      videoTitleMark.className = 'videoTitleMark';
      videoTitleMark.innerHTML = this.videotitle;
      if (this.elements.container.nextSibling && this.elements.container.nextSibling.className !== null && this.elements.container.nextSibling.className === 'videoTitleMark') return;   //防止图片说明多次添加
      this.elements.container.parentNode.insertBefore(videoTitleMark, this.elements.container.nextSibling);
  ...
  var media = {
    ...
    setup: function setup() {
      ...
      if (this.isVideo) {
        ...
        //this.elements.wrapper.style.height = this.elements.poster.style.height = getStyle(this.elements.poster).width.slice(0, -2) * 9 / 16 + 'px';
      }
  ...
  var Plyr =
    ...
    function () {
      function Plyr(target, options) {
        var _this = this;
        //记录title  jack
//      this.videotitle = target.getAttribute('title') ? target.getAttribute('title') : '请规范流程，写上标题';
 		this.videotitle = target.getAttribute('title') ? target.getAttribute('title') : '';
        ...
        if (!is.element(this.elements.container)) {
          this.elements.container = createElement('div');
          wrap(this.media, this.elements.container); //jack
